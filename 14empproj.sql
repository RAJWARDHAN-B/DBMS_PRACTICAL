...
(102, 'Bob', 60000, '2345678901', 'Mumbai', 2),
(103, 'Charlie', 70000, '3456789012', 'Pune', 1),
(104, 'David', 65000, '4567890123', 'Pune', 1),
(107, 'Eve', 55000, '5678901234', 'Delhi', 1);
...
-- 1. Details of employees working on project 353 and 354
SELECT e.*
FROM emp e
JOIN assigned_to a ON e.eno = a.eno
WHERE a.pno IN (353, 354);

-- 2. Employees working on the Database Project
SELECT e.*
FROM emp e
JOIN assigned_to a ON e.eno = a.eno
...
-- 3. Employee nos who work on at least one project that employee 107 works on
SELECT DISTINCT a2.eno
FROM assigned_to a1
JOIN assigned_to a2 ON a1.pno = a2.pno
WHERE a1.eno = 107 AND a2.eno != 107;

-- 4. Employee nos who work on ALL projects that employee 107 works on
SELECT e1.eno
FROM emp e1
WHERE NOT EXISTS (
    SELECT pno
    ...
    WHERE eno = 107
    AND pno NOT IN (
        SELECT pno FROM assigned_to WHERE eno = e1.eno
    )
)
AND e1.eno != 107;

-- 5. Project with minimum number of employees
...
-- 6. View: pno, pname, number of employees per project
...
-- 7. Stored Procedure using Cursor: Show employees on a given project
DELIMITER //
